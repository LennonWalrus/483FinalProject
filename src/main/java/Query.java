...
//this program reads questions from question text file formats them and then uses them to create lucene queries to get MRR Scores using classic or tf idf simularity
public class Query {
    ...
    public static void main(String[] args) {
        ...
        queries = queryClean(queries);// make the question/hint strings cleaned
        luceneAnswers(queries);//use  hints to gen queries to calc MRR
    ...
    //method to clean generated query strings
    private static ArrayList<String> queryClean(ArrayList<String> old){
        ArrayList<String> stemed = new ArrayList<>();// list of cleaned queries
        for(String pre: old){//looping though each query string
            stemed.add(LineCleaner(pre));//add cleaned string to cleaned queries
        }
        return stemed;
    }

    //method to remove punctuation and stop words from query so they can be used with the query parser
    ...
    //method to generate lucene query from cleaned strings and then use query to gen MRR Scores for each question
    private static void luceneAnswers(ArrayList<String> queries){
        ...
        for(String quest: queries) {//loop through stemmed queries
            ...
            System.out.println(quest);//print the cleaned query without the ORs
            String[] tokens = quest.split("[!.,?;\\s]+");//split the cleaned query
            ...
            try {
                ...
                searcher.setSimilarity(new ClassicSimilarity());// set searcher to use classic similarity which makes the scoring tf IDF instead of bm25
