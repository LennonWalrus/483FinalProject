...
public class Query {
    ...
    static ArrayList<String> answers2 = new ArrayList<>();//list of second answers to questions if any
    ...
    public static void main(String[] args) {
       ...
       queries = queryLemm(queries);// make the question/hint strings into lemmas
    ...
    private static ArrayList<String> genQueries(Path questions){
        ...
        while(inputScanner.hasNext()){//looping though question file
            ...
            if(answer.contains("|")){//if answer contains two tiles
                String[] found = answer.split("\\|");//split
                answers.add(found[0]);//and add the two
                answers2.add(found[1]);
            }
            else{//if only one answer
                answers.add(answer);//add the one answer
                answers2.add("none");//leave second empty
            }
    ...
    //method to lemmatize generated query strings
    private static ArrayList<String> queryLemm(ArrayList<String> old){
        ArrayList<String> stemed = new ArrayList<>();// list of lemmatized queries
        for(String pre: old){//looping though each query string
            String clean = LineCleaner(pre);//clean the line of punctuation
            //System.out.println(clean);
            //System.out.println(clean.split("\\s+").length);
            if(clean.length() != 0) {//if line not empty
                Sentence sent1 = new Sentence(clean);//create a lucene sentence object with clean string
                //System.out.println(sent1.tokens());
                String lemmas = " ";//for lemmas
                for (String lemm : sent1.lemmas()) {//loop through lemmas of clean string
                    lemmas += lemm + " ";//add to lemmas string
                }
                //System.out.println(lemmas);
                stemed.add(lemmas);//add lemas string to lemmatized queries
            }
        }
        return stemed;
    }

    //method to remove punctuation and stop words from query strings
    private static String LineCleaner(String line){
        ...
        for(String s : tokens){//for each token
            ...
            //remove these symbols from the end of the token until there are no more of them
    ...
    //method to generate lucene query from lemmatized strings and then use query to gen MRR Scores for each question
    private static void luceneAnswers(ArrayList<String> queries){
        ...
        for(String quest: queries) {//loop through lemmatized queries
            count++;//up the answer list count
            String qstr = "content:";//start query stirng with content:
            System.out.println(quest);//print the lemmatized query without the ORs
            String[] tokens = quest.split("[!.,?;\\s]+");//split the lemmatized query
            for(String token: tokens){//for each
                if(token.equals(""))//if empty skip
                    continue;
                qstr += token + " OR content:";//other wise add lemma with trailing or and content to set up for next lemma
            }
            ...
            try {
                ...
                if(!answers2.get(count).equals("none")){//when second answer isnt none print it as well
